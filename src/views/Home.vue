<template>
  <div>
      ...
      <router-link :to='{path:"/adresslist"}'>
        <ul class="adrLeft mr8">
          <li class="flex Start pb9 ">
            ...
            <span class="font16 fw6 c3">{{ adrMes.Mname?adrMes.Mname:"请添加常用收货人" }} <span 
              class="fw5 c6 ml3"
    ...
    <div 
      class="p30 flex Start " 
      id="adres" 
      v-else 
      @click='goLogin'>
      <img 
        src='../assets/images/homepage/home_unlogged_pic@2x.png' 
        alt="" 
        width="60">
      <ul class="tl ml10">
    ...
    <ul class="shopList p30">
      <li 
        class="list flex Start mb10" 
        v-for='(item,index) in homeMes' 
        :key='index'
        @click.stop='goDetails(item.Gid,item.Gcid)' 
        :msg='detailObj'>
        <img 
          :src="item.Gurl" 
          alt="" 
        ...
        <ul class="ml10 listMes flex Between">
          ...
          <span class="font14 color">￥</span>{{ item.Gxsprice }}</span>
            <img 
              src="../assets/images/homepage/unshop_icon_png@2x.png" 
              alt="" 
              class="shopCar_Img"
              @click.stop='addShopCars(item.Gid,index)' 
              v-if='item.Gshopcarnums == 0' >
            <img 
              src="../assets/images/homepage/s_shop_icon_png@2x.png" 
              alt="" 
              class="shopCar_Img" 
              v-else >
          </li>
    ...
    <footer>
      ...
      <van-submit-bar 
        ...
        <div @click='goShopCar'> 
          <img 
            src="../assets/images/homepage/dingshui_xiangzi@2x.png" 
            alt="" 
            class="car ml10" >
        ...
        methods: {
            ...
            addShopCars(pid, index) {
                if (this.users) {
                    ...
                    let i = this.adrMes.SGnums;
                    getShop(obj).then(res => {
                        this.$toast(res.msg);
                        this.homeMes[index].Gshopcarnums = 1;
                        if(res.status ==1 ){
                            i++;
                            this.$set(this.adrMes,'SGnums',i)
                        }
